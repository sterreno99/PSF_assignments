...
def makeIntoCountour(posPred, fPredNorm, Nplot=50):
    ...
    fPlotNorm = griddata((posPred[0, :], posPred[1, :]), fPredNorm[0, :], 
                         (P0plot, P1plot), method="linear")
...
def makeGaussianProcessMagneticFieldMapPlots(magnetometerPositions, posPred, magneticFieldPredictions, magneticFieldPredictionsCovariance, magnetomerNorm, modelParameters):
    ...
    data = np.load('magneticFieldMap.npz')
    posPlotX2, posPlotY2, fPlotNormCov2 = makeIntoCountour(posPred, np.diag(magneticFieldPredictionsCovariance).reshape(1, -1))
    posPlotX2, posPlotY2, fPlotNorm2 = makeIntoCountour(posPred, magneticFieldPredictions.reshape(1, -1))
    fPlotNormCov2 = linAlg.normaliseArray(fPlotNormCov2)

    Vmin = np.min([np.min(data['fPlotNorm']), np.min(magnetomerNorm)])
    Vmax = np.max([np.max(data['fPlotNorm']), np.max(magnetomerNorm)])

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

    sc = ax1.contourf( data['posPlotX'], data['posPlotY'], data['fPlotNorm'], levels=50, vmin = Vmin, vmax = Vmax, cmap='viridis')
    ...
    sc = ax2.contourf(posPlotX2, posPlotY2, fPlotNorm2, levels=50, vmin = Vmin, vmax = Vmax, cmap='viridis')
...
def testKernelValue(x1, x2):
    if x1.ndim == 1:
        x1 = x1[:, np.newaxis]
    if x2.ndim == 1:
        x2 = x2[:, np.newaxis]

    numDim1, numInputs1 = x1.shape
    numDim2, numInputs2 = x2.shape

    if numDim1 != numDim2:
        raise ValueError("Input dimensions do not match")
    
    if numInputs1 != 1 or numInputs2 != 1:
        raise ValueError("Expects single input column vectors")
    
def testKernelMatrix(x1, x2):
    if x1.ndim == 1:
        x1 = x1[:, np.newaxis]
    if x2.ndim == 1:
        x2 = x2[:, np.newaxis]
